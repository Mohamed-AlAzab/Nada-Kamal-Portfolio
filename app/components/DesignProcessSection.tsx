...
import { Sparkles, Palette, Figma, Users, ArrowRight, Mail, Linkedin, Github, Award, BookOpen } from 'lucide-react';
  ...
  const processes = [
    {
      ...
      description: "Understanding user needs through interviews, surveys, and competitive analysis to build empathy",
      color: "from-pink-400 to-rose-400"
    },
    {
      icon: <Palette className="w-8 h-8" />,
      title: "Ideate & Design",
      description: "Creating wireframes, mockups, and high-fidelity designs that solve user problems beautifully",
            ...
            {processes.map((process, index) => (
              <div key={index} className="group relative">
                <div className="bg-white rounded-3xl p-8 border-2 border-pink-100 hover:border-pink-300 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
                  <div className={`w-16 h-16 bg-gradient-to-br ${process.color} rounded-2xl flex items-center justify-center text-white mb-6 group-hover:scale-110 transition-transform duration-300`}>
                    {process.icon}
                  </div>
